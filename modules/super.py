...
def _lucky_arch():
    return resolve_binary("lucky-arch")
...
def _unpack_fallback(image_path: Path, work_dir: Path, out_dir: Path) -> OperationResult:
    """Fallback: unpack super.img menggunakan simg2img + lpunpack."""
    lpunpack = _lpunpack()
    simg2img = _simg2img()
    ...
    # Hapus raw hasil konversi supaya hemat storage
    if raw_img != source_img:
    ...
    if not copied:
        return OperationResult(
            ok=False,
            title="Unpack super.img",
            message="super.img berhasil di-unpack tapi tidak ada partisi .img yang dihasilkan.",
    ...
    return OperationResult(
        ok=True,
        title="Unpack super.img",
        message=f"super.img berhasil di-unpack. ({len(copied)} partisi)",
...
def unpack_super(image_path: Path) -> OperationResult:
    ...
    lucky_arch = _lucky_arch()

    if lucky_arch:
        # Prioritas: lucky-arch (satu langkah, sparse otomatis)
        try:
            _clear_dir(work_dir)
            safe_mkdir(out_dir)
            ...
            if common.DEBUG:
                print(f"  [DEBUG] lucky-arch {source_img.name} -> {work_dir}")
            print(f"  [INFO] Menggunakan lucky-arch untuk unpack...")
            proc = subprocess.run(
                [str(lucky_arch), "-o", str(work_dir), str(source_img)],
                cwd=str(work_dir),
                check=False,
                capture_output=True,
                text=True,
            ...
            if not copied:
                return OperationResult(
                    ok=False,
                    title="Unpack super.img",
                    message="lucky-arch selesai tapi tidak ada partisi .img yang dihasilkan.",
                    output_path=str(out_dir),
                )
    ...
    # Fallback: simg2img + lpunpack
    print(f"  [INFO] lucky-arch tidak ditemukan, fallback ke simg2img + lpunpack...")
    try:
        _clear_dir(work_dir)
        safe_mkdir(out_dir)
        return _unpack_fallback(image_path, work_dir, out_dir)
