...
def raw_to_sparse(raw_path: Path, sparse_path: Path, block_size: int = 4096) -> bool:
    ...
    try:
        ...
        max_uint32 = 4294967295
        chunk_total_sz = chunk_hdr_sz + padded_len
        if blocks > max_uint32 or chunk_total_sz > max_uint32:
            size_gb = data_len / (1024**3)
            print(f"  [WARN] Image terlalu besar untuk sparse format ({size_gb:.1f}GB), tetap menggunakan raw")
            return False
