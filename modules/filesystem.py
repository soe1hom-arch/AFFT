...
def unpack_filesystem(image_path: Path, output_base: Path | None = None) -> OperationResult:
    ...
    try:
        ...
        if is_sparse_image(source_img):
            lucky_arch = resolve_binary("lucky-arch")
            raw_img = TEMP_DIR / f"{source_img.stem}_raw.img"
            if raw_img.exists():
                raw_img.unlink()

            if lucky_arch:
                print(f"  [INFO] Terdeteksi sparse image, mengonversi dengan lucky-arch...")
                convert = subprocess.run(
                    [str(lucky_arch), "--to-raw", str(source_img), str(raw_img)],
                    check=False, capture_output=True, text=True,
                )
                tool_name = "lucky-arch"
            else:
            ...
            if convert.returncode != 0:
                return OperationResult(
                    ok=False,
                    title=f"Unpack {image_path.name}",
                    message=f"Gagal konversi sparse dengan {tool_name}: {convert.stderr.strip() or 'unknown error'}",
                    output_path=str(out_dir),
                )
